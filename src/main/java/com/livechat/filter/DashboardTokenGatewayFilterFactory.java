 ...
 */
@Component
public class DashboardTokenGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> implements Ordered {

	private static final Logger log = LoggerFactory.getLogger(DashboardTokenGatewayFilterFactory.class);
	...
	public DashboardTokenGatewayFilterFactory() {
		super(Config.class);
	}
	...
	public void init() {
		this.webClient = WebClient.builder()
				.baseUrl(GatewayConstant.DASHBOARD_URL)
	...
	private Mono<Void> setUidInHead(ServerWebExchange exchange, GatewayFilterChain chain, Boolean optional) {
		...
		log.info("dashboard setUidInHead uri={}", path);

		if(path.indexOf(GatewayConstant.DASHBOARD_URL_PATH) == GatewayConstant.ZERO){
			return mobileClientResponse(exchange,chain,optional,path);
		}
		return chain.filter(exchange);
	}

	private Mono<Void> mobileClientResponse(ServerWebExchange exchange, GatewayFilterChain chain, Boolean optional, String path) {

		return webClient
				.post()
				.uri(GatewayConstant.DASHBOARD_CHECK_TOKEN_URL)
				...
				.flatMap(result -> {
					if (result != null && result.isSuccess()) {
						...
						builder.headers((httpHeaders) -> {
							...
							httpHeaders.remove(GatewayConstant.STORE_ID);
							httpHeaders.add(GatewayConstant.UID, String.valueOf(sessionResp.get(GatewayConstant.UID)));
							httpHeaders.add(GatewayConstant.STORE_ID, String.valueOf(sessionResp.get(GatewayConstant.STORE_ID)));
						});
